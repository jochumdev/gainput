...
GAINPUT_COMPILE_ASSERT(sizeof(PS4Controller) <= HID_CONTROLLER_BUFFER_SIZE);
...
GAINPUT_COMPILE_ASSERT(sizeof(PS4CInputReport) == 42);
// GAINPUT_COMPILE_ASSERT(offsetof(PS4CInputReport, gyro) == 12);
...
GAINPUT_COMPILE_ASSERT(sizeof(PS4CEffectsState) == 19);
...
static int HIDUpdatePS4Controller(HIDController* controller, gainput::InputDeltaState * state)
{
    ...
    for (; (size = hid_read_timeout(dev, data, PS4_BT_INPUT_REPORT_SIZE, 0)) > 0;)
    {
        ...
        {
            GAINPUT_ASSERT(size <= PS4_USB_INPUT_REPORT_SIZE);
            HIDParsePS4ControllerInputReport(con, (PS4CInputReport*)(data + 1), state);
        }
        else
        {
            GAINPUT_ASSERT(*data >= PS4_EXT_INPUT_REPORT_ID_FIRST && *data <= PS4_EXT_INPUT_REPORT_ID_LAST);
            GAINPUT_ASSERT(size <= PS4_BT_INPUT_REPORT_SIZE);
